...
QueryDX11::QueryDX11(QueryType queryType, uint8_t numBuffers)
	: m_QueryType(queryType)
	, m_NumBuffers(numBuffers)
{
	m_pDevice = RendererDX11::getInstance().getDevice();
	m_pDeviceContext = RendererDX11::getInstance().getDeviceContex();
	...
	for (uint8_t i = 0; i < m_NumBuffers; ++i)
	{
		...
		{
			LogManager::getInstance().log("Failed to create Query object.");
		}
	}
	...
	if (m_QueryType == Query::QueryType::Timer)
	{
		...
		for (uint8_t i = 0; i < m_NumBuffers; ++i)
		{
			...
			{
				LogManager::getInstance().log("Failed to create timer query object.");
			}
		}
	}
}
...
Query::QueryResult QueryDX11::GetQueryResult(int64_t frame)
{
	...
	if (buffer >= 0)
	{
		if (m_QueryType == QueryType::Timer)
		{
			//第二个参数为null值检查状态，返回false表示被检测对象当前不可用
